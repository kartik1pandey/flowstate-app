    ...
    try {
      ...
      if (existingUser.rows.length > 0) {
        if (existingUser.rows[0].email_verified) {
          ...
          return res.json({
            message: 'Verification email resent. Please check your email (or console in dev mode).',
            emailSent: true,
            userId: userId
          });
        }
      ...
      res.status(201).json({
        message: 'Registration successful! Please check your email to verify your account (or check console in dev mode).',
        user: {
          id: user.id,
          email: user.email,
          name: user.name,
...
router.post('/resend-verification', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      res.json({
        message: 'Verification email sent! Please check your inbox (or console in dev mode).',
        emailSent: true
      });
    } finally {
      client.release();
    ...
    try {
      ...
      if (!user.email_verified) {
        return res.status(403).json({ 
          error: 'Please verify your email before signing in. Check your email or console for verification link.',
          emailVerified: false,
          email: user.email
        });
      }
