...
import { sessionsAPI, executeAPI } from '@/lib/api-client-new';
...
export default function CodeEditorSpace() {
    ...
    try {
      ...
      await sessionsAPI.create({
        ...
        metrics: {
          ...
          tabSwitches: 0,
          mouseActivity: totalKeystrokes,
          fatigueLevel: 0,
        },
        triggers: ['Coding session'],
        breakers: [],
        interventions: [],
        notes: `${language} coding session`,
      });
      
      console.log('Code session saved successfully');
    } catch (error) {
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      const result = await executeAPI.run(language, code);
      
      // Check if backend wants client-side execution
      if (result.clientSideExecution && (language === 'javascript' || language === 'typescript')) {
      ...
      if (result.exitCode !== 0 && !result.clientSideExecution) {
  ...
  const executeClientSide = () => {
    try {
      ...
      setOutput(output || '✅ Code executed successfully in browser (no output)');
    } catch (evalError: any) {
            ...
            <div className="mt-6 pt-6 border-t border-gray-700">
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between text-xs mb-2">
                  ...
                  <span className="text-white font-semibold">
                    {Math.round((keystrokesPerMinute * 0.3) + (flowScore * 0.7))}%
                  </span>
                </div>
                <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-cyan-500 to-blue-500 transition-all duration-500"
                    style={{ width: Math.round((keystrokesPerMinute * 0.3) + (flowScore * 0.7)) + '%' }}
                  />
                </div>
                <p className="text-xs text-gray-500 mt-1">
