...
export default function WhiteboardAnalytics() {
  ...
  const loadSessions = async () => {
    try {
      ...
      // Filter for whiteboard sessions only
      const filtered = allSessions.filter((s: any) => {
        const sessionDate = new Date(s.startTime);
        const isInRange = now.getTime() - sessionDate.getTime() < timeFilters[timeRange];
        const isWhiteboard = s.sessionType === 'whiteboard' || s.whiteboardMetrics;
        return isInRange && isWhiteboard;
      });
  ...
  const avgCreativity = sessions.reduce((sum, s) => sum + (s.whiteboardMetrics?.creativityScore || 0), 0) / (totalSessions || 1);
  const avgFocusScore = sessions.reduce((sum, s) => sum + (s.focusScore || 0), 0) / (totalSessions || 1);
  const totalStrokes = sessions.reduce((sum, s) => sum + (s.whiteboardMetrics?.totalStrokes || 0), 0);
  const totalShapes = sessions.reduce((sum, s) => sum + (s.whiteboardMetrics?.shapesDrawn || 0), 0);
  const avgCanvasCoverage = sessions.reduce((sum, s) => sum + (s.whiteboardMetrics?.canvasCoverage || 0), 0) / (totalSessions || 1);
  const avgColorsUsed = sessions.reduce((sum, s) => sum + (s.whiteboardMetrics?.colorsUsed || 0), 0) / (totalSessions || 1);
    ...
    datasets: [
      {
        ...
        data: sessions.map((s) => s.whiteboardMetrics?.creativityScore || 0),
      ...
      {
        label: 'Focus Score',
        data: sessions.map((s) => s.focusScore || 0),
  ...
  const activityData = {
    ...
    datasets: [
      {
        label: 'Total Strokes',
        data: sessions.map((s) => s.whiteboardMetrics?.totalStrokes || 0),
        backgroundColor: 'rgba(99, 102, 241, 0.8)',
      },
      {
        label: 'Shapes Drawn',
        data: sessions.map((s) => s.whiteboardMetrics?.shapesDrawn || 0),
    ...
    datasets: [
      {
        ...
        data: sessions.map((s) => ({
          x: s.whiteboardMetrics?.creativityScore || 0,
          y: s.focusScore || 0,
        })),
