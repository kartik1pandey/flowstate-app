...
export default function WritingSpace() {
  ...
  useEffect(() => {
    ...
    if (elapsedMinutes > 0) {
      setTypingSpeed(Math.round(chars / elapsedMinutes));
    }
  }, [markdown, startTime]);

  // Removed flow monitoring - can be re-implemented later
  // useEffect(() => {
  //   if (isInFlow) {
  ...
  const handleStartSession = async () => {
    startFlowSession();
    setStartTime(Date.now()); // Reset timer when starting session
  };
      ...
      <div className="max-w-7xl mx-auto py-6 px-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            {/* Preview */}
            {showPreview && (
              <div className="bg-white rounded-xl shadow-sm p-8 min-h-[300px] max-h-[600px] overflow-auto">
                <h3 className="text-sm font-semibold text-gray-500 mb-4 uppercase">Preview</h3>
                <div className="prose prose-sm max-w-none !text-gray-900 [&_*]:!text-gray-900">
            ...
            {isInFlow && (
              <div className="bg-gradient-to-br from-primary/10 to-purple-600/10 rounded-xl shadow-sm p-4">
                ...
                <div className="space-y-3">
                  ...
                  <MetricBar
                    label="Writing Speed"
                    value={Math.min(100, (charCount / 10))}
                    color="bg-green-500"
                  />
                  <MetricBar
                    label="Consistency"
